...
def draw_header(canv, title=None, text_color=cfg.C_TEAL):
    ...
    if title:
        ...
        utils.draw_center(canv, cfg.font_s, 6, cfg.C_WHITE, f"{state.get('temp', '0')}\u00b0C")

        sig = state.get('wifi_signal', 0)
        if sig < -70 and sig != 0:
            graphics.DrawText(canv, cfg.font_s, 39, 6, cfg.C_WHITE, time.strftime("%H:%M"))
            graphics.DrawLine(canv, 60, 5, 60, 2, cfg.C_RED)
            graphics.DrawLine(canv, 61, 5, 61, 3, cfg.C_RED)
            graphics.DrawLine(canv, 62, 5, 62, 4, cfg.C_RED)
        else:
...
def draw_footer(canv):
    ...
    if notifications:
        valid = [n for n in notifications if n.get("expires", 0) > now]
        if valid:
            notif = valid[-1]

    msg_custom = state.get('msg_custom', '')
    ...
    for txt, color in parts:
        l = graphics.DrawText(canv, cfg.font_s, curr_x, 64, utils.to_matrix_color(color), txt)
        curr_x += l
        txt_len += l

    pos_scroll -= 1
    if pos_scroll + txt_len < 0:
        pos_scroll = 64
