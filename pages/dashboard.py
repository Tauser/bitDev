...
def draw(canv):
    ...
    # Animação
    speed = state.get('gif_speed', cfg.GIF_SPEED)
    ...
    w_num = utils.text_width(cfg.font_l, str_num)
    ...
    if qtd > 0:
        ...
        if base_i < qtd:
            m1 = sec[base_i]
            utils.draw_market_coin_column(canv, cfg, m1, 1, y_sym, y_prc, y_pct, 25)
        ...
        if base_i + 1 < qtd:
            m2 = sec[base_i + 1]
            off = 34
            utils.draw_market_coin_column(canv, cfg, m2, off, y_sym, y_prc, y_pct, off + 24)
        else:
            # Slot Vazio -> Saitama
            if frames_saitama:
