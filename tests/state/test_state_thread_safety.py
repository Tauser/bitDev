...
def test_state_snapshot_thread_safety_basic(monkeypatch):
    ...
    def fake_get(url, timeout=0, **_kwargs):
        if "BTCUSDT" in url:
            n = next(counter)
            return FakeResp({"lastPrice": str(10000 + n), "priceChangePercent": str((n % 10) - 5)})
    ...
    def writer():
        try:
            for _ in range(80):
                data.fetch_btc_only()
                data.add_notification("ok", duration=2)
                time.sleep(0.001)
...
def test_state_snapshot_is_stable_dict_shape(monkeypatch):
    data = _import_data_fresh(monkeypatch)

    snap = data.get_state_snapshot()
    assert isinstance(snap, dict)
    assert "bitcoin" in snap
    assert "status" in snap
    assert isinstance(snap.get("secondary", []), list)
